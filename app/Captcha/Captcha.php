...
class Captcha
{
    ...
    public function __construct(
        ...
        $this->fontsDirectory = config('captcha.fontsDirectory',  base_path() . '/app/Captcha/assets/fonts');
    }
    ...
    public function create(string $config = 'default', bool $api = false)
    {
        ...
        $this->backgrounds = $this->files->files(base_path() . '/app/Captcha/assets/backgrounds');
    ...
    public function check(string $value): bool
    {
        ...
        if (!$this->session->has('tentativas')) {
            $this->session->put('tentativas',0);
            $this->session->put('tentativas_limite',config('captcha')['tentativas']);
        }

        if($this->session->get('tentativas') < $this->session->get('tentativas_limite')){
            return true;
        }
        
        if (!$this->session->has('captcha')) {
            $this->session->increment('tentativas',1);
            return false;
        }
        ...
        if ($check) {
            ...
            $this->session->remove('tentativas');
        }
