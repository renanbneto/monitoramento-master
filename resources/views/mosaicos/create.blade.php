...
@section('title', 'Novo Mosaico')

@section('content_header')
    <h1><i class="fas fa-plus mr-2"></i>Novo Mosaico</h1>
@stop

@section('content')
<div class="row">
    <div class="col-md-12">
        <form method="POST" action="{{ route('mosaicos.store') }}">
            @csrf
            <div class="row">

                {{-- Painel esquerdo: dados do mosaico --}}
                <div class="col-md-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="nome" class="form-control @error('nome') is-invalid @enderror"
                                       value="{{ old('nome') }}" placeholder="Ex.: Entrada Principal" required>
                ...
                {{-- Painel direito: seleção de câmeras --}}
                <div class="col-md-8">
                    <div class="card card-secondary">
                        ...
                        <div class="card-body p-0" style="max-height:480px; overflow-y:auto;">
                            <table class="table table-sm table-hover mb-0">
                                ...
                                <tr class="camera-row" data-nome="{{ strtolower($camera->local_nome) }} {{ strtolower($camera->cidade) }}">
                                    <td>
                        ...
                        <div class="card-footer">
                            <small class="text-muted">Clique nas câmeras desejadas. A ordem de seleção define a posição no mosaico.</small>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 text-right">
                    <a href="{{ route('mosaicos.index') }}" class="btn btn-default mr-2">Cancelar</a>
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save mr-1"></i> Salvar Mosaico
                    </button>
                </div>
            </div>
        </form>
...
document.querySelectorAll('.camera-check').forEach(function (cb) {
    cb.addEventListener('change', function () {
        ...
        row.classList.toggle('selecionada', this.checked);
        atualizarContador();
    });
});
</script>
