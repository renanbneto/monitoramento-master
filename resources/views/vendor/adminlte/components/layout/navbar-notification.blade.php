...
<script>
    ...
    $(() => {
        ...
        nLink = new _AdminLTE_NavbarNotification("{{ $id }}");
        updateNotification(nLink);
...
<script>
    ...
    class _AdminLTE_NavbarNotification {
        ...
        update(data)
        {
            ...
            //Possui pelo menos uma obrigatória?
            if(data.notificacoes && data.notificacoes.some(notification => notification.leituraObrigatoria === true)){
                    //Vamos atualizar a session do usuario para notificacao_obrigatoria = count
                    const totalObrigatoria = data.notificacoes.reduce((contador, notification) => {
                        if (notification.leituraObrigatoria === true) {
                            return contador + 1;
                        }
                        return contador;
                    }, 0);
                    ...
                    $.ajax({
                        url:'{{ route('atualizaSessionNotificacoesObrigatorias') }}',
                        method: 'POST',
                        dataType: 'json',
                        data:{
                            notificacaoObrigatoria:totalObrigatoria
                        },
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        success: (data) => {


                            @if(!request()->routeIs('exibirNotificacoes'))
                            location.href = 'exibirNotificacoes';
                            @endif

                        },
                        err: (err) => {
                            toastr.error('O usuário dispõe de notificações obrigatórias, porém houve erro ao marcar a leitura obrigatória no sistema!')
                        }
                    })
                }else{
                    $.ajax({
                        ...
                        dataType: 'json',
                        data:{
                            notificacaoObrigatoria:0
                        },
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        success: (data) => {
                            //toastr.success('O usuário não dispõe de notificações obrigatórias, navegação será liberada!')
                        },
                        err: (err) => {
                            toastr.error('O usuário não dispõe de notificações obrigatórias, porém houve erro ao liberar a navegação no sistema!')
                        }
                    })
                }
            ...
            $(".adminlte-dropdown-content a.dropdown-item").click(function () {
                $(this).remove();            
            });
        }
    }
